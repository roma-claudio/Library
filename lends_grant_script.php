<?php
	...
	while ($row = pg_fetch_assoc($execute_query)) {	
		$query="UPDATE Prestito SET attivo='t' WHERE numregistrazionecopia='".$row["numregistrazionecopia"]."' AND emailutente='".$row["emailutente"]."' AND passwordutente='".$row["passwordutente"]."' AND datainizio='".$_POST["datainizio"]."'";
		//echo $query;
		$execute_query=pg_query($conn,$query);
		if ($execute_query){
